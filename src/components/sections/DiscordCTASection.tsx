"use client";
import { WobbleCard } from "@ui/wobble-card";

export const DiscordCTASection = () => {
  return (
    <section className="section-themed data-stream-bg relative overflow-hidden py-24">
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          {/* Discord Card - Large */}
          <WobbleCard
            containerClassName="col-span-1 lg:col-span-2 bg-purple-800 min-h-[300px] lg:min-h-[400px]"
            className=""
          >
            <div className="max-w-lg">
              <h2 className="text-left text-2xl font-semibold tracking-tight text-balance text-white md:text-3xl lg:text-4xl">
                Join our Discord Community
              </h2>
              <p className="mt-4 text-left text-base/6 text-purple-200">
                Connect with 750+ cybersecurity enthusiasts. Get instant updates on events, ask questions, share
                resources, and network with fellow students and industry professionals.
              </p>
              <div className="mt-6">
                <a
                  href="https://discord.com/invite/duca"
                  className="landing-card-button-secondary inline-flex items-center justify-center rounded-lg bg-purple-600 px-6 py-3 font-semibold text-white transition-colors hover:bg-purple-700"
                >
                  <svg className="mr-2 h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028 14.09 14.09 0 0 0 1.226-1.994.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z" />
                  </svg>
                  Discord
                </a>
              </div>
            </div>
            <div className="absolute -right-4 -bottom-10 h-48 w-48 opacity-20">
              <svg viewBox="0 0 24 24" fill="white" className="h-full w-full">
                <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028 14.09 14.09 0 0 0 1.226-1.994.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z" />
              </svg>
            </div>
          </WobbleCard>

          {/* Join Club Card */}
          <WobbleCard containerClassName="col-span-1 bg-cyan-800 min-h-[300px]">
            <h2 className="text-left text-2xl font-semibold tracking-tight text-balance text-white md:text-xl lg:text-2xl">
              Become a Member
            </h2>
            <p className="mt-4 text-left text-base/6 text-cyan-200">
              Join DUCA through DUSA and unlock exclusive benefits, workshops, and networking opportunities.
            </p>
            <div className="mt-6">
              <a
                href="https://dusa.org.au/clubs/deakin-university-cybersecurity-association-burwood-duca"
                className="landing-card-button-primary inline-flex items-center justify-center rounded-lg bg-cyan-600 px-6 py-3 font-semibold text-white transition-colors hover:bg-cyan-700"
              >
                Regsiter
              </a>
            </div>
          </WobbleCard>
        </div>
      </div>
    </section>
  );
};

export default DiscordCTASection;
